...
**Álgebra: la ecuación escondida en el enunciado.** En `ch.alg.hidden_equation_ages` nadie escribe una ecuación; el jugador tiene que nombrar la caja, escribir dos edades futuras y darse cuenta de que "será el doble" es una balanza. La construcción es armar la balanza. En `ch.alg.symmetric_sum_of_squares` lo oculto es una identidad: el cuadrado de una suma como cuatro baldosas, de las que dos son el producto conocido. El mismo patrón baja hasta los primeros nodos: en `ch.alg.chained_scales_hidden_weight` ninguna de las dos pesadas alcanza sola y el peso aparece al restar una balanza de la otra; en `ch.alg.second_equation_in_the_picture` la segunda ecuación no está escrita, está en la hilera de baldosas que entra justa; en `ch.alg.garden_wider_and_narrower` lo oculto es que el lado del cuadrado no hace falta, porque la tira que se agrega y la que se quita se cancelan salvo un cuadradito de esquina; en `ch.alg.meter_hidden_flat_fee` y `ch.alg.read_the_inverse_off_the_graph` el dato oculto es la pendiente, que solo aparece al armar el escalón entre dos puntos; en `ch.alg.machine_inside_a_machine` la regla de la primera máquina se recupera pasando cada salida al revés por la llave de la segunda.

**Teoría de números: divisibilidad y restos.** En `ch.arith.two_clocks_same_position` dos engranajes esconden un sistema de congruencias; la construcción es listar y alinear. En `ch.arith.parity_invariant_coins` lo oculto es un invariante: nada de lo que el jugador haga cambia la paridad, y descubrirlo es la respuesta. En `ch.arith.same_remainder_hidden_divisor` el divisor no está: aparece porque dos totales con el mismo resto tienen una diferencia que sí se divide justo. En `ch.arith.one_more_makes_it_divide` lo oculto no es un número sino un reencuadre: todos los sobrantes están a uno de cerrar, y agregar una bolita vuelve exactas las tres repartijas. Más abajo en la espina el mismo patrón se juega sin leer: `ch.arith.where_the_parentheses_went` esconde la agrupación, `ch.arith.hidden_numerator_on_the_line` el numerador, `ch.arith.ledger_with_hidden_debts` una fila con signo, y `ch.arith.covered_row_count` y `ch.arith.pair_them_to_compare` esconden cuánto hay y cuál tiene más.
...
**Trigonometría: ángulos en la rueda y leyes.** `ch.trig.unit_circle_hidden_cosine` es Pitágoras sobre el triángulo del radio, y el dato oculto es el signo, que decide el cuadrante. `ch.trig.which_law_hidden_side` esconde la decisión: qué ley conviene según el patrón de datos, y siempre queda el camino de bajar la altura y usar Pitágoras dos veces. `ch.trig.all_solutions_on_the_wheel` esconde la segunda solución de cada rama; `ignored_case` se explica reflejando el ángulo en la rueda. `ch.trig.wheel_mark_height_from_arc` esconde el ángulo mismo: no lo dan, sale de dividir el arco recorrido por el radio, y recién entonces la altura es el radio por el seno.

**Fundamentos: la construcción antes de los símbolos.** Los nodos 1 a 12 también tienen desafíos, y son los que más se apoyan en el gesto: emparejar, prolongar un espaciado, anotar el total corrido al costado de cada fila. `ch.arith.covered_row_count` y `ch.arith.pair_them_to_compare` son enteros sin lectura; `ch.alg.chained_scales_hidden_weight` cruza la balanza de prealgebra con la primera ecuación de un paso.

**Cálculo: optimización con restricción escondida.** En `ch.calc.fence_along_the_river` el área depende de dos lados y el cerco los ata; `ignored_constraint` se dispara cuando el jugador deriva con dos variables libres, y el caminante muestra que la colina no tiene cima si no se fija la restricción. `ch.calc.cylinder_inside_a_cone` es el cruce más fuerte del catálogo: la restricción es geométrica y se descubre con `construct` (corte axial, triángulos semejantes) antes de derivar. Los dos desafíos de la definición hacen lo mismo con el límite: en `ch.calc.instant_rate_between_two_chords` la velocidad de un instante no está en ningún lado y aparece achicando la cuerda de los dos lados; en `ch.calc.tank_from_the_rate_graph` el caudal tapado se recupera prolongando el tramo recto, y recién ahí el área bajo el gráfico es el agua acumulada.

**Álgebra lineal: transformación compuesta.** En `ch.linalg.matrix_of_a_composed_move` lo oculto es adónde van las flechas base después de dos movimientos, y que el orden importa. En `ch.linalg.shadow_gives_the_height` la altura de un paralelogramo se descubre proyectando: la misma perpendicular del trapecio, ahora sobre vectores, y el determinante como comprobación. Antes de las matrices están las flechas: en `ch.linalg.the_leg_that_closes_the_walk` el tramo tapado es el que cierra la cadena punta con cola, y en `ch.linalg.count_the_moves_to_the_treasure` lo oculto son las coordenadas del tesoro en la grilla torcida que generan las dos movidas, que además puede no contenerlo.

**Geometría elemental: el ángulo que aparece por construcción.** En `ch.geom.zigzag_between_parallels` el ángulo del codo no toca ninguna de las dos paralelas y por eso ninguna regla se le aplica; la recta auxiliar trazada por el vértice lo parte en dos ángulos que sí son alternos internos de los datos.
...
Los desafíos existen también en los niveles 0 a 2, y no requieren leer ([Q](../Q-edad-universal.md), sección 4.2). Son **rompecabezas con datos ocultos**: una figura o una fila cubierta parcialmente, donde hay que descubrir cuánto hay usando lo que se ve. El catálogo inicial tiene doce, con `literacy_min: none`:
...
- `ch.arith.covered_row_count`: una fila de cajas con el medio tapado por una cinta; repetir por debajo el hueco que hay entre dos cajas visibles cuenta las que faltan.
- `ch.arith.pair_them_to_compare`: dos montones desordenados; tirar un puente desde cada objeto de uno hasta uno del otro dice cuál tiene más sin contar ninguno.
- `ch.arith.hidden_numerator_on_the_line`: un punto en la recta con el numerador tapado; cortar el entero en tantas partes iguales como dice el denominador lo revela.
- `ch.arith.ledger_with_hidden_debts`: un libro de cuentas con una fila tapada; el salto entre el total de antes y el de después da el signo y el tamaño de lo que decía.
- `ch.arith.where_the_parentheses_went`: una cuenta que no da lo que dice al lado; deshacer la última operación sobre el resultado dice cuánto tiene que valer el grupo de adentro.

Estos desafíos cumplen las reglas de Q sin excepción: el enunciado es una sola frase para voz (o nada, si el sonido está apagado; la figura con la hoja que tapa es el enunciado), los gestos son `drag` y `hold` con targets grandes, la cheatsheet es una imagen con voz, las tres pistas son animaciones y la solución paso a paso es una animación narrada. Cuentan en K igual que cualquier otro desafío: un niño que descubre las baldosas tapadas produce un ítem `transfer` de `geom.area.rect_and_triangle`, y uno que empareja dos montones para decidir cuál tiene más produce uno de `found.count.cardinality`. El tiempo no se muestra, y `T_STAR_LITERACY_NONE_FACTOR` se aplica como en todo nodo sin lectura.
...
Derivada de [`challenges.yaml`](challenges.yaml). La construcción clave es la primera de `hidden_data`; los caminos completos están en el archivo. Setenta y nueve desafíos: geometría 14 (5 sin leer), álgebra 12, aritmética y teoría de números 13 (6 sin leer), combinatoria 6, probabilidad 6, trigonometría 8, cálculo 9, álgebra lineal 5, grafos 6 (1 sin leer).
...
| `ch.geom.zigzag_between_parallels` | geom | regional | `geom.angle.turn_as_measure`<br>`geom.angle.parallel_transversal`<br>`geom.cons.auxiliary_lines` | `the_line_that_relates_the_two_arms`<br>`the_two_parts_of_the_middle_angle` | `draw_through_the_vertex_a_line_parallel_to_the_two_given_ones` |
...
| `ch.alg.chained_scales_hidden_weight` | alg | entrenamiento | `prealg.eq.balance`<br>`prealg.var.unknown_as_box`<br>`alg.eq.one_step` | `weight_of_one_cube`<br>`weight_of_the_sphere` | `take_the_second_balance_away_from_the_first_pan_by_pan` |
| `ch.alg.meter_hidden_flat_fee` | alg | entrenamiento | `alg.fn.linear_slope`<br>`alg.fn.graph_as_picture`<br>`prealg.var.unknown_as_box` | `price_per_kilometre`<br>`the_flat_fee` | `build_the_rise_over_run_staircase_between_the_two_trips` |
| `ch.alg.machine_inside_a_machine` | alg | regional | `alg.fn.composition`<br>`alg.fn.function_as_machine`<br>`prealg.inv.operation_as_key` | `the_middle_values`<br>`rule_of_the_hidden_machine` | `run_the_known_machine_backwards_on_each_output` |
| `ch.alg.garden_wider_and_narrower` | alg | regional | `alg.expr.distributive_tiles`<br>`geom.area.rect_and_triangle`<br>`alg.eq.one_step` | `the_side_of_the_square`<br>`the_corner_tile_that_does_not_fit` | `notice_that_the_change_in_area_does_not_depend_on_it` |
| `ch.alg.second_equation_in_the_picture` | alg | regional | `alg.sys.two_by_two`<br>`alg.expr.distributive_tiles`<br>`prealg.eq.balance` | `the_second_equation`<br>`size_of_one_tile` | `read_the_row_of_tiles_that_fits_exactly_across_the_frame` |
| `ch.alg.read_the_inverse_off_the_graph` | alg | regional | `alg.fn.inverse_function`<br>`alg.fn.graph_as_picture`<br>`alg.fn.linear_slope` | `the_slope_of_the_line`<br>`the_value_of_the_inverse` | `build_the_rise_over_run_triangle_between_two_lattice_points` |
...
| `ch.arith.covered_row_count` | arith (sin leer) | entrenamiento | `found.count.cardinality`<br>`found.count.number_line`<br>`arith.add.displacement` | `boxes_under_the_strip`<br>`total_boxes` | `step_the_visible_spacing_under_the_strip` |
| `ch.arith.pair_them_to_compare` | arith (sin leer) | entrenamiento | `found.count.cardinality`<br>`arith.sub.undo_add` | `which_group_has_more`<br>`leftover_after_pairing` | `draw_one_bridge_from_each_object_to_a_partner` |
| `ch.arith.hidden_numerator_on_the_line` | arith (sin leer) | entrenamiento | `arith.frac.parts_and_ratio`<br>`found.count.number_line`<br>`arith.div.undo_mul` | `the_numerator`<br>`length_of_one_equal_part` | `cut_the_unit_into_as_many_equal_parts_as_the_denominator` |
| `ch.arith.ledger_with_hidden_debts` | arith (sin leer) | entrenamiento | `arith.int.negatives`<br>`arith.add.displacement`<br>`arith.sub.undo_add` | `sign_of_the_covered_row`<br>`size_of_the_covered_row` | `compare_the_running_total_before_and_after_it` |
| `ch.arith.where_the_parentheses_went` | arith (sin leer) | regional | `arith.expr.precedence_tree`<br>`arith.mul.scaling`<br>`arith.add.displacement` | `value_of_the_inner_chest`<br>`the_grouping` | `undo_the_outer_operation_on_the_given_result` |
| `ch.arith.same_remainder_hidden_divisor` | arith | regional | `arith.div.undo_mul`<br>`arith.sub.undo_add`<br>`disc.mod.clock_equivalence` | `the_divisor`<br>`which_candidates_survive` | `subtract_the_two_totals_and_factor_the_difference` |
| `ch.arith.one_more_makes_it_divide` | arith | internacional | `arith.div.undo_mul`<br>`arith.div.remainder`<br>`arith.num.divisible_or_not`<br>`disc.mod.clock_equivalence` | `the_reframe_add_one`<br>`first_common_multiple_of_all_the_divisors` | `notice_every_remainder_is_one_short_of_its_divisor` |
...
| `ch.trig.wheel_mark_height_from_arc` | trig | regional | `trig.circle.unit_circle_radians`<br>`geom.angle.turn_as_measure`<br>`trig.fn.sine_as_height` | `the_turned_angle`<br>`the_height_above_the_axle` | `divide_the_arc_the_mark_travelled_by_the_radius` |
...
| `ch.calc.instant_rate_between_two_chords` | calc | internacional | `calc1.deriv.rate_as_slope_limit`<br>`precalc.lim.approach`<br>`alg.fn.linear_slope` | `the_slope_of_each_chord`<br>`the_instantaneous_rate` | `draw_the_chord_to_a_neighbour_on_each_side_and_build_its_rise_run_triangle` |
| `ch.calc.tank_from_the_rate_graph` | calc | internacional | `calc1.int.accumulation`<br>`geom.area.rect_and_triangle`<br>`alg.fn.graph_as_picture` | `the_rate_under_the_panel`<br>`the_area_of_the_covered_piece` | `extend_the_straight_segment_across_the_covered_stretch` |
...
| `ch.linalg.the_leg_that_closes_the_walk` | linalg | regional | `linalg.vec.vector_as_displacement`<br>`linalg.vec.span_and_combination`<br>`alg.fn.graph_as_picture` | `the_hidden_leg`<br>`the_length_of_that_leg` | `close_the_chain_of_arrows_from_the_last_tip_back_to_the_finish` |
| `ch.linalg.count_the_moves_to_the_treasure` | linalg | internacional | `linalg.vec.span_and_combination`<br>`linalg.vec.vector_as_displacement`<br>`alg.sys.two_by_two` | `whether_the_target_can_be_reached_at_all`<br>`how_many_of_each_move` | `check_that_the_two_arrows_are_not_multiples_of_each_other` |
