...
Sin entrada en el catálogo, y por eso sin clasificar ni bloquear `ready`: confundir estirar con desplazar (el distractor de `explain` y el gesto que el clavo bloquea; el juego reproduce los dos movimientos superpuestos con las marcas iluminadas) y sumar los dos factores (el juego coloca esa cantidad de baldosas en el marco y queda un hueco, como `missing_piece_tiles` sobre `tiles`).
