...
2. Demostración: una mano fantasma mantiene apretado el grifo. El tanque sube y abajo se dibuja la curva de nivel. La mano suelta, arrastra al caminante hasta un instante de la curva de nivel y aparece el poste con su pendiente. El poste y la aguja del caudal marcan lo mismo, y los dos se iluminan a la vez. La escena vuelve al inicio.
3. El jugador llena el tanque y arrastra al caminante por la curva de nivel. En cada instante el poste coincide con la aguja de arriba. Donde el caudal es máximo, la curva de nivel es la más empinada; donde el grifo está cerrado, el nivel es plano.
4. El jugador da vuelta el grifo. La curva de nivel baja y el poste del caminante se da vuelta con ella. La coincidencia se mantiene con signo.
5. Aparece el cofre. La cerradura tiene la forma del caminante midiendo la cuesta; el llavero ofrece cuatro llaves, una de las cuales es el tanque acumulando. El jugador arrastra una llave. La correcta gira y el cofre se abre mostrando la curva de nivel; las otras se traban.
6. El cofre abierto muestra el diagrama vertical del nodo 12, con las mismas flechas: arriba la curva de nivel, la flecha hacia abajo con la cerradura, abajo la curva de caudal, y la llave como la misma flecha hacia atrás.
7. El jugador toca las dos marcas del cronómetro. En la curva de nivel se encienden dos alturas y la diferencia entre ellas queda medida con una regla vertical. Arriba, el área entre las dos marcas se llena. Los dos números coinciden.
8. El jugador arrastra la curva de nivel entera hacia arriba. La curva sube pero el caminante mide lo mismo en todos lados y el área de arriba no cambia. Aparecen varias copias de la curva, apiladas y paralelas: la familia.
9. Éxito: el cofre queda abierto con el diagrama y la regla vertical al lado. Sin cartel.
...
La **barra de evaluación**, con las dos marcas arriba y abajo, nace de un problema de repetición: para calcular una integral definida hay que escribir la expresión de la primitiva dos veces, una con cada marca, y en cuanto la expresión tiene más de dos términos eso ocupa dos renglones y se presta a errores de copia. La barra dice "esta expresión, en el de arriba menos en el de abajo" escribiéndola una sola vez.

El **`+ C`** nace de un problema distinto y más incómodo: la llave no devuelve un cofre sino una familia. Derivar destruye la altura inicial del tanque, igual que elevar al cuadrado destruye el signo, y por eso la operación inversa devuelve todas las alturas posibles a la vez. `C` es el nombre de lo que la llave no puede saber, y el juego lo introduce en el momento en que el jugador arrastra la curva de nivel hacia arriba y ve que el caminante mide lo mismo. Que la diferencia entre dos marcas no dependa de `C` es lo que hace que la integral definida sí tenga un valor único, y el jugador lo comprueba arrastrando la familia entera mientras la regla vertical no se mueve.
...
- `explain`: arrastrar la llave de acumular al cofre de la derivada y elegir, entre tres animaciones, la que muestra por qué la altura del tanque en cada instante es justo el caudal acumulado. Una muestra el poste del caminante coincidiendo con la aguja; otra, el nivel que copia la forma del caudal en vez de su pendiente; otra, el área que se toma como si fuera la altura de la curva de caudal. Tocar la correcta.
- `manipulate`: llenar el tanque siguiendo la curva de caudal y comprobar, con el caminante sobre la curva de nivel, que la pendiente del nivel coincide con el caudal en tres instantes distintos, incluido uno con el grifo dado vuelta.
...
Las dos analogías se retiran en `formal`. Antes, en `symbolic`, el auto ya no aparece y el tanque perdió el grifo: quedan las dos curvas sobre ejes, la de caudal arriba y la acumulada abajo, con el caminante como herramienta. El cofre se queda como fantasma a demanda hasta `formal`, porque representa estructura y no un objeto.
...
- `calc2.tech.substitution_undoes_chain` (`chest_key` y `grid_stretch`): la sustitución como llave de segundo nivel. El cofre tiene otro cofre adentro, y la llave de afuera es la que deshace la composición del nodo 27.
- `adv.four.inverse_transform` (`chest_key` y `machine_pipe`): la transformada y su inversa como máquina que se corre al revés, donde la información que se pierde y se recupera es otra y la estructura de llave es la misma.
- `prob.rv.cumulative_distribution` (`fill_accumulate` y `urn_dice`): la densidad y la acumulada. La pendiente de la acumulada es la densidad, que es este teorema con otro nombre y sin cálculo a la vista.
- `adv.ode.euler_method` (`fill_accumulate`, `gears_sequence` y `slope_walker`): reconstruir una curva a partir de su pendiente, paso a paso. Es la llave usada a mano, con error, y es donde el `+ C` se vuelve el dato inicial que hace falta para arrancar.
...
**Visualización:** dos escenas del YAML, resueltas en [I](../../I-manim/I0-mapping.md). `chest_scene_integral_key_opens_derivative` es de ruta mixta: pre-renderizada para la apertura de cada nivel y nativa para dibujarse sobre las curvas del jugador. La cerradura muestra su forma de caminante midiendo la cuesta, el llavero rota, la llave de acumular entra y gira, y el diagrama vertical del nodo 12 se reproduce hacia atrás; gramática `invert`, parametrizada por la función y el intervalo. Es la imagen de cheatsheet de `cs.calc1.strategy_integral_is_a_key`. `fill_scene_accumulation_rate_equals_height` es nativa: las dos gráficas apiladas, el área que se rellena arriba mientras la curva de nivel se dibuja abajo, y el caminante con su tangente cuya lectura coincide con la altura de la curva de arriba; gramática `accumulate` con `rate` encima, parametrizada por la función de caudal, la de nivel y el instante. Produce las animaciones de `explain`. Se reúsan las escenas de rectángulos del nodo 28 y la del caminante del nodo 26 como distractores. Ninguna lleva texto rasterizado: `∫`, la barra de evaluación, `C` y los valores los dibuja el runtime según el locale ([P](../../P-internacionalizacion.md)).

**Calculadora:** en `ready` se habilita `op_antiderivative` ([M](../../M-calculadora/M0-progresion.md)), un ícono de llave sobre el signo de integral, junto a cualquier función armada con fichas. No devuelve solo el número: escribe la primitiva con su `+ C`, después el renglón con la barra de evaluación y las dos marcas, y recién entonces el valor. Es la primera operación simbólica de integración del juego, y convive con `op_integral` del nodo 28, que sigue disponible en su forma numérica para verificar. Si el nodo decae, el ícono muestra óxido y la calculadora vuelve a la respuesta numérica.
