...
**Calculadora:** en `ready` se habilita `op_solve_linear` ([M](../../M-calculadora/M0-progresion.md)), un ícono de llave junto a cualquier igualdad con una incógnita armada con fichas. No devuelve solo `x = 7`: escribe el renglón con la operación bajo los dos lados y después el resultado. Si el nodo decae, la llave muestra óxido.
